...
Si copiamos la URL en el navegador, nos funciona porque el navegador es lo bastante listo para traducir espacios y diferenciar las comillas pero en R, con la función curl_download, necesitamos usar URLencode para que funcione.
```{r}
...
```
Si presiono control + enter, ejecuta la línea y pasa a la siguiente  eso lo ejecuta en el diario de trabajo.

En el segundo bloque solo hay un par de líneas y es mas corto pero en el tercer bloque es mas claro y más práctico, si qiero hacer una búsqueda diferente es más fácil modificar.
:::
...
No hay que cambiar el portal y limit porque al ejecutar el bloque anterior por lo que no lo cambiamos. En el guion se ejecuan las cosas detrás de otras si ya he definido el término portal ya ha quedado definido

```{r}
adh <- read.table('ADH.txt', sep ='\t', quote = "", header = TRUE)
```
El problema es que utiliza algún carácter como abrir comillas y esto mete en las comillas miles de líneas y nos da error. Añadimos el quote.
Hemos generado un marco de datos que se llama adh que incorpora el ADH.txt
