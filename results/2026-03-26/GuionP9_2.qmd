...
Con la función glance(mtdna) poodemos confirmar el número de secuencias: 42.
La longitud del alineamiento es de 16603 (apartado nchar de glance(mtdna)).
Con la función names(mtdna) podemos extraer los nombres de las secuencias.
...
Con la función class(mt_nj) consultamos la clase del objeto. Lo mismo con mt_upgma. Ambos son de clase "phylo"
...
plot(mt_nj, type = "phylogram", use.edge.length = TRUE,
     ...
     FALSE)
```
Al consultar la ayuda de la función plot(mt_nj) nos sale algunas de las opciones que podemos cambiar para la representación gráfica. Si no tocamos nada, obtenemos el mismo árbol que antes
...
axishylo: Esta función añade un eje  al lado de un gráfico filogenético.
:::
...
Hay algunas distancias evolutivas que parecen no cuadrar las con las dataciones. Estas especies parecen mas antiguas de lo que realmente son
...
Es posible que la datación de esos organismos sea imprecisa. Estas secuencias podrian contener cambios que no son mutaciones ancestarles sino producto de la degradacuion del adn desde que se depositó hasta que se analizó. Se supone que hay métodos estadisticos para corregirlo pero así parece que los denisovanos han acumulado más cambios que los que realmente han acumulado. Esto se puede deber a degradación o errores de datación
:::
...
0.09104153/0.01214282 = 7.497561
7,5 veces
